...
class TangoFluxDEG(TangoFlux):
    ...
    def compute_capo_loss(self, audio_latent, text, event_description, duration, co_rewards):

        co_win, co_lose = co_rewards
        batch_size = len(text)
        ...
        target_preference = co_win - co_lose
    ...
    def forward(self, latents, prompt, event_description=None, duration=torch.tensor([10]), sft=True, precomputed_preferences=None):
        ...
        if sft: 
            ...
            if precomputed_preferences is not None:
                ...
                copo_loss = (target_preference - self.beta_dpo * preference_diff).pow(2).mean()
                
                loss = copo_loss + 0.1 * model_losses_w.mean()
